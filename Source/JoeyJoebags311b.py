...
class Window(Frame):
    ...
    def init_window(self):
        ...
        GBA_GenericMenu.add_command(label='Dump 64kbytes FLASH/SRAM', command=main_GBA_Dump64kSRAM)
        GBA_GenericMenu.add_command(label='Dump 128kbytes FLASH/SRAM', command=main_GBA_Dump128kFLASH)
...
def main_GBA_Dump128kFLASH():
    ...
    if RAMfileName:
        RAMfile = open(RAMfileName, 'wb')
        Address = 0
        for Address in range(0, 131072, 64):
            Lo = Address & 255
            Me = (Address & 65280) >> 8
            Hi = (Address & 16711680) >> 16
            dev.write(1, [53, 0, Hi, Me, Lo])
...
def main_ELCheapo_Write():
    ...
    for BankNumber in range(0, int(ROMsize / 16384)):
        ...
        for ROMAddress in range(16384, 32768, 32):
            ...
            AddLo = ROMAddress & 255
            Data32Bytes = ROMbuffer[ROMpos:ROMpos + 32]
            for addoffset in range(0, 32):
                DataByte = ROMbuffer[ROMpos + addoffset]
                AddLo2 = AddLo + addoffset
                Addlo = AddLo + addoffset
                dev.write(1, [10, 1, 4, 10, 170, 170, 5, 85, 85, 10, 170, 160, AddHi, Addlo, DataByte])
                RAMbuffer = dev.read(129, 64)
...
def NES_Dump():
    global ROMbuffer
    ROMfileName = asksaveasfilename(defaultextension='.NES', filetypes=(('NES ROM File', '*.NES'),
                                                                        ('All Files', '*.*')))
    if ROMfileName:
        ROMfile = open(ROMfileName, 'wb')
        for bankNumber in range(0, 63, 2):
            dev.write(1, [17, 0, 0, 128, 0, 6])
            ROMbuffer = dev.read(129, 64)
            dev.write(1, [17, 0, 0, 128, 1, bankNumber])
            ROMbuffer = dev.read(129, 64)
            dev.write(1, [17, 0, 0, 128, 0, 7])
            ROMbuffer = dev.read(129, 64)
            dev.write(1, [17, 0, 0, 128, 1, bankNumber + 1])
            ROMbuffer = dev.read(129, 64)
            for ROMaddress in range(0, 16383, 64):
                AddHi = ROMaddress >> 8
                AddLo = ROMaddress & 255
                dev.write(1, [16, 0, 0, AddHi, AddLo])
                ROMbuffer = dev.read(129, 64)
